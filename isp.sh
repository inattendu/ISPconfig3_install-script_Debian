...
/etc/init.d/hostname.sh >> isp.log
echo "    --> Actualisation"
echo "---------------------------"
...
dpkg-reconfigure -f noninteractive dash >> isp.log
echo "---------------------------"
...
sed -i 's|# -o smtpd_sasl_auth_enable=yes| -o smtpd_sasl_auth_enable=yes|' /etc/postfix/master.cf
sed -i 's|# -o smtpd_client_restrictions=permit_sasl_authenticated,reject| -o smtpd_client_restrictions=permit_sasl_authenticated,reject|' /etc/postfix/master.cf
/etc/init.d/postfix restart >> isp.log
...
/etc/init.d/clamav-daemon start >> isp.log
/etc/init.d/spamassassin stop >> isp.log
update-rc.d -f spamassassin remove >> isp.log
...
tar xvfz jailkit-2.17.tar.gz >> isp.log
cd jailkit-2.17
./debian/rules binary >> isp.log
cd ..
dpkg -i jailkit_2.17-1_*.deb >> isp.log
rm -rf jailkit-2.17*
...
ignoreregex =
EOF
/etc/init.d/fail2ban restart >> isp.log
echo ""
